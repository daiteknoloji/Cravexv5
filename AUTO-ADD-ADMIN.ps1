        ...
        if ($newRooms.Count -gt 0) {
            ...
            foreach ($roomId in $newRooms) {
                try {
                    ...
                    # Admin API ile zorla ekle - user_id BODY'de olmalı!
                    ...
                    if ($errorMsg -like "*already*" -or $errorMsg -like "*M_FORBIDDEN*") {
                        ...
                        Write-Host "                 [ATLA] Sifre korumalı/Ozel oda (davet gerekli)" -ForegroundColor Yellow
                        $joinedRooms += $roomId  # Tekrar denemesin
                    } elseif ($errorMsg -like "*404*" -or $errorMsg -like "*Not Found*") {
                        Write-Host "                 [ATLA] Oda bulunamadi (silinmis olabilir)" -ForegroundColor DarkGray
                        $joinedRooms += $roomId  # Tekrar denemesin
                    } else {
